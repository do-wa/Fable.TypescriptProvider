...
let emitFsFileAsJson (fsPath: string) (fsFileOut: FsFileOut) =
    let file = fs.createWriteStream (fsPath + ".json")
    let json = JS.JSON.stringify(fsFileOut)
    file.write(json) |> ignore
    file.``end``()
    []
...
let emitFsFileOut fsPath (fsFileOut: FsFileOut) =
    emitFsFileAsJson fsPath fsFileOut
